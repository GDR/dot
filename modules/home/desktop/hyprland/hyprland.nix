...
let
  ...
  # Generate a Lua theme module required by hyprland.lua
  # Returns a table with color values that hyprland.lua uses.
  hyprTheme = ''
    -- Generated by Nix — do not edit.
    -- Change theme by updating theme.name in your host config.
    -- Theme: ${theme.displayName}
    return {
      border_active   = "${rgba r.borderFocused "ee"}",
      border_inactive = "${rgba r.border "66"}",
      shadow_color    = "${rgba r.background "cc"}",
      shadow_inactive = "${rgba r.background "99"}",
    }
  '';
...
lib.my.mkModuleV2 args {
  ...
  module = {
    nixosSystems = {
      ...
      # hyprland.lua loads it with dofile() to pick up color values.
